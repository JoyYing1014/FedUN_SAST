...
class FedUN(fs.UnlearnAlgorithm):
	def __init__(self,
		...
		super().__init__(name, data_loader, module, device, train_setting, client_num, client_list, online_client_num, save_model, max_comm_round,
	...
	def update_subspace(self, g_locals):
		"""
		子空间更新逻辑 (Warm-up & Online Update)
		包含 Procrustes 对齐和平滑更新
		"""
		if len(g_locals) == 0:
			return
		...
		# 构造贡献矩阵 Z = sum(g_i * c_i) = G.T @ C  (D, k)
		Z = g_stack.T @ c
	...
	def train_a_round(self):
		...
		# 条件：1. 设置了更新频率 > 0
		#       2. 当前轮次符合频率要求
		#       3. 有保留梯度可用 (g_ret_list 非空)
		if self.u_update_freq > 0 and \
			(self.current_comm_round + 1) % self.u_update_freq == 0 and \
			len(g_ret_list) > 0:
			# print(f"Updating Subspace at round {self.current_comm_round}...")
